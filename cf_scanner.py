...
CONNECT_TIMEOUT  = 3.0
XPAD_TIMEOUT     = 5.0   # таймаут на каждый HEAD в X-Pad тесте

# ТСПУ: диапазон обрыва 
TSPU_MIN_KB = 1
TSPU_MAX_KB = 69

# X-Pad тест: сколько HEAD-запросов, размер каждого куска мусора
XPAD_CHUNKS    = 18       # ~72 КБ суммарно (18 × 4 KB) — перекрывает диапазон ТСПУ
XPAD_CHUNK_KB  = 4        # КБ мусора в каждом запросе
XPAD_SLEEP     = 0.03     # пауза между запросами (сек)
...
# Мусорный пул для X-Pad (генерируем один раз)
import string as _string
_RAND_POOL = "".join(
    random.choices(_string.ascii_letters + _string.digits, k=200_000)
)
...
async def _step2_xpad(domain: str) -> Tuple[bool, str]:
    """
    Шаг 2: X-Pad keepalive тест.
    Серия HEAD-запросов с нарастающим мусорным заголовком через одно
    keepalive соединение. Провоцируем ТСПУ независимо от размера сайта.

    Возвращает (ok: bool, reason: str).
    ok=True — ТСПУ не обнаружен.
    ok=False — обрыв в диапазоне TSPU_MIN_KB..TSPU_MAX_KB или иная ошибка.
    """
    try:
        import httpx
    except ImportError:
        # httpx не установлен — пропускаем тест, не блокируем домен
        return True, "httpx_not_installed"
    ...
    try:
        async with httpx.AsyncClient(
            ...
            http2=False,
            limits=limits,
            timeout=httpx.Timeout(XPAD_TIMEOUT, connect=CONNECT_TIMEOUT),
        ) as client:
            url = f"https://{domain}/"
            base_headers = {
            ...
            for i in range(XPAD_CHUNKS):
                headers = base_headers.copy()
                if i > 0:
                    start_idx = random.randint(0, len(_RAND_POOL) - chunk_size - 1)
                    headers["X-Pad"] = _RAND_POOL[start_idx: start_idx + chunk_size]

                t0 = time.time()
                try:
                    await client.request("HEAD", url, headers=headers)
                    elapsed = time.time() - t0

                    if i == 0:
                        rtt = elapsed
                    elif rtt is not None:
                        # Если ответ занял аномально долго — возможно ТСПУ замедляет
                        if elapsed > max(rtt * 5, 2.0):
                            kb_sent = (i * XPAD_CHUNK_KB)
                            return False, f"tspu_slow:{kb_sent}KB"

                    await asyncio.sleep(XPAD_SLEEP)

                except Exception as e:
                    kb_sent = i * XPAD_CHUNK_KB
                    err = classify_error(e, kb_sent * 1024)
                    return False, f"{err}@xpad:{kb_sent}KB"

            return True, ""

    except Exception as e:
        return False, f"xpad_init:{classify_error(e)}"
...
async def check_domain(
    ...
    """
    Полная проверка домена: резолвинг → GET / → X-Pad → TLS.
    ...
    # ── Шаг 2: X-Pad keepalive ────────────────────────────────────────────────
    xpad_ok, xpad_reason = await _step2_xpad(domain)
    result["xpad_ok"]     = xpad_ok
    result["xpad_reason"] = xpad_reason

    if not xpad_ok:
        result["dead_reason"] = xpad_reason
        result["elapsed"] = time.time() - t_start
        return result
...
class ResultWriter:
    ...
    def add(self, r: dict):
        ...
        self._log_lines.append(
            ...
            f"xpad={'ok' if r['xpad_ok'] else r['xpad_reason']} "
...
async def mode_check(args):
    ...
    # Шаг 2
    if r["xpad_ok"] is True:
        print(f"  X-Pad       ✓ ТСПУ не обнаружен")
    elif r["xpad_ok"] is False:
        print(f"  X-Pad       ✗ ТСПУ детектирован: {r['xpad_reason']}")
    else:
        print(f"  X-Pad       — (не дошли до этого шага)")
